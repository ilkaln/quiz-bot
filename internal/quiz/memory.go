...
type MemoryStorage struct {
	...
	QuizIDMap    map[string]*Quiz
	OwnerIDmap   map[int64][]*Quiz
	RunIDMap     map[string]*QuizRun
	RunQuizIDMap map[string][]*QuizRun
...
func NewMemoryStorage() *MemoryStorage {
	return &MemoryStorage{
		QuizIDMap:    make(map[string]*Quiz),
		OwnerIDmap:   make(map[int64][]*Quiz),
		RunIDMap:     make(map[string]*QuizRun),
		RunQuizIDMap: make(map[string][]*QuizRun),
	}
}

// SaveQuiz сохраняет квиз.
func (s *MemoryStorage) SaveQuiz(ctx context.Context, q *Quiz) error {
...
func (s *MemoryStorage) GetQuiz(ctx context.Context, id string) (*Quiz, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
...
func (s *MemoryStorage) ListQuizzes(ctx context.Context, ownerID int64) ([]*Quiz, error) {
	if err := ctx.Err(); err != nil {
		return nil, err
	}
...
func (s *MemoryStorage) SaveRun(ctx context.Context, run *QuizRun) error {
	if err := ctx.Err(); err != nil {
		return err
	}
	...
	if _, ok := s.RunQuizIDMap[run.QuizID]; !ok {
		s.RunQuizIDMap[run.QuizID] = make([]*QuizRun, 0)
	}
...
func (s *MemoryStorage) GetRun(_ context.Context, id string) (*QuizRun, error) {
	s.mute.Lock()
	defer s.mute.Unlock()
...
func (s *MemoryStorage) ListRuns(_ context.Context, quizID string) ([]*QuizRun, error) {
	s.mute.Lock()
	defer s.mute.Unlock()
...
func (s *MemoryStorage) UpdateRun(ctx context.Context, run *QuizRun) error {
	if err := ctx.Err(); err != nil {
		return err
	}
...
func (s *MemoryStorage) GetOwnerQuizzes(ownerID int64) []*Quiz {
	s.mute.Lock()
	defer s.mute.Unlock()
...
func (s *MemoryStorage) SetQuizOwner(ownerID int64, q *Quiz) {
	s.mute.Lock()
	defer s.mute.Unlock()
	if q == nil {
		return
	...
	if _, ok := s.OwnerIDmap[ownerID]; !ok {
		s.OwnerIDmap[ownerID] = make([]*Quiz, 0)
	}
