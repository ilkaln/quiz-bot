...
type QuizInputData struct {
	Title     string     `json:"title"`
	Settings  Settings   `json:"settings"`
	Questions []Question `json:"questions"`
}

type LobbyRun struct {
...
type QuizRun struct {
	...
	setQuestionReaponses []map[int64]struct{}
	CurrentQuestionIdx   int
	ChStopRun            chan (struct{})
	QuestionsSlicePtr    *[]Question
	QuestionsStartsAt    []time.Time
	StartedAt            time.Time
	FinishedAt           time.Time
	ResultsCSVBytes      *[]byte
}
...
type QuizEngine interface {
	...
	GetPersQuizzes(persID int64) []*Quiz

	SetQuizOwner(ownerID int64, quiz *Quiz)

	GetQuiz(quizID string) (*Quiz, error)

	GetListRuns(quizID string) ([]*QuizRun, error)
}
